...
const TextArea: React.FC<TextareaProps> = ({
  ...
  if (disabled) {
    textareaClasses += ` bg-gray-100 opacity-50 text-gray-500 dark:text-gray-300 border-gray-300 cursor-not-allowed dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700`;
  } else if (error) {
      ...
      {hint && (
        <p
          className={`mt-2 text-sm ${
            error ? 'text-error-500' : 'text-gray-500 dark:text-gray-300 dark:text-gray-400'
          }`}
        >
          {hint}
        </p>
